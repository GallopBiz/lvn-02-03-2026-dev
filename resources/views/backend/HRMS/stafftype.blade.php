            ...
            <div class="row">
                <div class="col-md-3 form-group mb-3">
                    ...
                    <input 
                        ...
                        type="text"
                        value="{{ $stream_master->staff_type_name ?? '' }}"
                ...
                <div class="col-md-3 form-group mb-3">
                    ...
                    <select required name="shift_type_id" id="shift" class="form-control">
                        @foreach($shiftTypes as $shiftType)
                            <option value="{{ $shiftType->id }}" 
                                {{ isset($stream_master) && $stream_master->shift == $shiftType->shift_type_name ? 'selected' : '' }}>
                            ...
                            <tbody>
                                @if(!empty($stream))
                                    @foreach($stream as $index => $streams)
                                        <tr>
                                            <td>{{ $index + 1 }}</td>
                                            <td class="uperletter">{{ $streams->staff_type_name }}</td> 
                                            <td class="uperletter">{{ $streams->shiftType->shift_type_name }}</td> 

                                            <td class='d-flex'>
                                                <a class="btn btn-primary m-1" href="{{ url('view-stafftype/' . $streams->id) }}">Edit</a>
                                                <?php $a = $streams->id; ?>
